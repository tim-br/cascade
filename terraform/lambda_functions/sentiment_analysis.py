...
def lambda_handler(event, context):
    ...
    if random.random() < failure_rate:
        ...
        print(f"SIMULATED FAILURE: {error_msg}")
        ...
        return {
            ...
            "body": {
                ...
                "note": "This is an intentional failure for testing error handling"
            }
        }
    ...
    try:
        ...
        chapter_sentiments = [
            {"chapter": 1, "positive": 0.45, "negative": 0.25, "neutral": 0.30, "overall": "positive"},
            {"chapter": 2, "positive": 0.35, "negative": 0.40, "neutral": 0.25, "overall": "negative"},
            {"chapter": 3, "positive": 0.50, "negative": 0.20, "neutral": 0.30, "overall": "positive"},
            {"chapter": 4, "positive": 0.30, "negative": 0.50, "neutral": 0.20, "overall": "negative"},
            {"chapter": 5, "positive": 0.55, "negative": 0.15, "neutral": 0.30, "overall": "positive"}
        ]

        # Aggregate sentiment
        avg_positive = sum(ch['positive'] for ch in chapter_sentiments) / len(chapter_sentiments)
        avg_negative = sum(ch['negative'] for ch in chapter_sentiments) / len(chapter_sentiments)
        avg_neutral = sum(ch['neutral'] for ch in chapter_sentiments) / len(chapter_sentiments)
        ...
        print(f"Sentiment analysis complete: overall={overall_sentiment}, score={result['sentiment_score']}")
