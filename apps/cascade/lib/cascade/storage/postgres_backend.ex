defmodule Cascade.Storage.PostgresBackend do
  @moduledoc """
  Postgres storage backend implementation.

  Uses Postgres-specific features:
  - Row-level locking (FOR UPDATE) for atomic task claiming
  - JSONB storage for definition, context, and result fields
  ...
  def claim_task(job_id, task_id, worker_id) do
    # Use Postgres-specific row-level locking for atomicity
    Repo.transaction(fn ->
  ...
  def upsert_worker_heartbeat(node, capacity, active_tasks) do
    ...
    # Use Postgres-specific ON CONFLICT for upsert
    %WorkerHeartbeat{}
